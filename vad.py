...
def VAD(signal, sr, nFFT=512, win_length=0.025, hop_length=0.01, threshold=0.7):
    ...
    # probRatio = np.repeat(probRatio, np.ceil(len(signal)/len(probRatio)))
    # probRatio = probRatio[:len(signal)]
    return probRatio

if __name__ == '__main__':
    path_wav = os.path.join(os.getcwd() , "datasets/SI1265_FJWB0_2.wav" )
    (sr, signal) = wav.read(path_wav)
    vad=VAD(signal, sr, nFFT=512, win_length=0.025, hop_length=0.01, threshold=0.99)
    sigmax = max(signal)
    signal = [samp/sigmax for samp in signal]
    plt.plot(signal)
    plt.plot(vad)
    plt.show()
